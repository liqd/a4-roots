...
def test_newsletter_email_has_no_default_greeting():
    """
    Ensure that the newsletter email does not render the default greeting
    from the global email base template.
    """
    user = UserFactory()
    ...
    subject, txt, html = email.render(email.template_name, context)
...
def test_newsletter_email_uses_organisation_logo_as_inline_logo(tmp_path):
    """
    Ensure that when an organisation logo is present, it is attached with
    Content-ID <logo>, matching the cid:logo reference in the email template.
    """
